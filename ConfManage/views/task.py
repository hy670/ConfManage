...
def task_model(request):
    if request.method == "GET":
        ...
        for task in list(keys(cTasks)):
            if task.startswith('ConfManage.tasks.cron') or task.startswith('ConfManage.tasks.sched'):
...
@login_required
@permission_required('ConfManage.read_periodictask',login_url='/noperm/')
