...
def assets_search(request):
	...
	if request.method == "GET":
		...
		for a in assetsList:
			...
			if a.status == 0:
				status = '''<button  type="button" class="btn btn-outline btn-success">已上线</button>'''
			elif a.status == 1:
				status = '''<button  type="button" class="btn btn-outline btn-primary">已下线</button>'''
			elif a.status == 2:
				status = '''<button  type="button" class="btn btn-outline btn-warning">维修中</button>'''
			elif a.status == 3:
				status = '''<button  type="button" class="btn btn-outline btn-info">已入库</button>'''
			elif a.status == 4:
				status = '''<button  type="button" class="btn btn-outline btn-default">未使用</button>'''
			if a.buy_time:
