...
from ConfManage.models import db_change_permission
from django.http import QueryDict

@login_required()
@permission_required('ConfManage.views_users', login_url='/noperm/')
def user_manage(request):
	if request.method == "GET":
		...
		if request.POST.get('op') == 'change_permission':
			permissionsList = [
				'views_assets',
				'views_topo',
				'views_policy',
			...
			return JsonResponse({'result':changeResult})
		elif request.POST.get('op') == 'add_user':
			userinf = json.loads(request.POST['userinfo'])
			if userinf['password'] == userinf['c_password']:
				try:
					user = User.objects.filter(username=userinf['username'])
					if len(user) > 0:
						return JsonResponse({"code": 500, "data": None, "msg": "注册失败，用户已经存在。"})
					else:
						user = User()
						user.username = userinf['username']
						user.email = userinf['email']
						user.is_staff = 0
						user.is_active = 0
						user.is_superuser = 0
						user.set_password(userinf['password'])
						user.save()
